...
class ExcelProcessor:
    ...
    def generate_xml_for_mwe_short(self, data=None):
        ...
        TYLKO z typami A01 i P01 (bez limitów dyspozycyjności)
        ...
        try:
            ...
            # PPLAN Series - businessType A01 zgodnie z przykładem (bez podkreślnika)
            ...
            # Sprawdź czy kolumna PAUTO istnieje i ma wartości
            has_pauto_column = 'Nadwyżki (PAUTO)' in data.columns
            has_pauto_values = False
            
            if has_pauto_column:
                has_pauto_values = data['Nadwyżki (PAUTO)'].notna().any() and (data['Nadwyżki (PAUTO)'] != 0).any()
            
            # Dodaj PAUTO Series tylko jeśli są rzeczywiste wartości
            if has_pauto_column and has_pauto_values:
            ...
            # UWAGA: Usunięto A_60 (PMAXLIM) i A_61 (PMINLIM) 
            # Te typy są dla planów dyspozycyjności (LONG/MWE), nie dla planów generacji (SHORT/MWE)
            ...
            logger.info(f"Series: PPLAN (A01){', PAUTO (P01)' if has_pauto_column and has_pauto_values else ''}")
            logger.info(f"UWAGA: Usunięto A_60 i A_61 - są to typy dla planów dyspozycyjności (LONG/MWE)")
